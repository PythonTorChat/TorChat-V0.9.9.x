...
class Buddy(object):
    ...
    def onProfileName(self, name):
        ...
        if self.name == "" and name <> "":  #JM - This is a one-shot fill in a blank name.
            self.name = name
            self.bl.save()
    ...
    def onProfileText(self, text):
        ...
        if self.version > "0.9.9.553":
            self.dbstatus = text[0:1]
    ...
    def sendProfile(self):
        if self.isAlreadyPonged():
            ...
            #JM - Send the blanks. User should have ability to delete this from remote end
            if name <> "":
                msg = ProtocolMsg_profile_name(self, name.encode("UTF-8"))
            else:
                msg = ProtocolMsg_profile_name(self, " ")
            msg.send()
    ...
    def getDisplayName(self):
        #JM - DO NOT TOUCH!! A fair bit of hard-coded logic depends on this one.
        if self.name == u"":
            line = self.address
        else:
            line = self.address + u" (" + self.name + u")"
        return line
    ...
    def __init__(self, callback, socket=None):
        ...
        for line in l:
            ...
            if len(line) > 15:
                ...
                try:
                    ...
                    addy = line
                if len(addy) == 17:
                    self.dpstatus = addy[16:17]   # Python "slicing"
                else:
                    self.dpstatus = STATUS_GOOD
        ...
        for buddy in self.list:
            if buddy.address == config.get("client", "own_hostname"):
                found = True  #JM - This is us
                if buddy.name == u"":
                    buddy.name = config.get("profile", "name")
                    if buddy.name == u"":
                        buddy.name = u"Myself"  #JM - Local end not allowed to be a blank
                        config.set("profile", "name", buddy.name)
        ...
        if not found:
            ...
            if config.get("client", "own_hostname") != "0000000000000000":
                if name == u"":
                    name = u"myself"    #JM - keep the old TorChat default behavior
...
# JM - Not called anywhere - Queued for deletion
