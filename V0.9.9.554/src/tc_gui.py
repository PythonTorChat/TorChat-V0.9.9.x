...
#    				(c) 2014-2019 M. Weigand   <doctortor@use.startmail.com>         #
#                                                                            #
...
# this is a graphical User interface for the tor.chat client library.
...
class TaskbarIcon(wx.TaskBarIcon):
    ...
    def getToolTipText(self):
        text = "tor.chat: %s" % config.getProfileLongName()
...
class DlgEditProfile(wx.Dialog):
    ...
    def onOk(self, evt):
        ...
        for buddy in self.mw.buddy_list.list:
            #JM FIXME - also stuff profile name into buddy list for myself or revert to "myself" if null
            buddy.sendProfile()
            buddy.sendStatus()
...
class BuddyList(wx.ListCtrl):
    ...
    def onListChanged(self):
        ...
        #JM FIXME - Put own address in top line always, then a seperator, then buddies
        #JM - Clear the list and re-populate it from the buddylist file
...
#            title += u" (%s)" % self.buddy.address #JM - reads better in windows
...
#        return t[:-19]     # no longer works as hard-coded trim.
    ...
    def writeColored(self, color, name, text, stat):
    ...
    def notifyOfflineSent(self):
        ...
        self.writeColored(config.get("gui", "color_nick_myself"), config.getProfileName(), message, self.buddy.dpstatus)
    ...
    def process(self, message):
        ...
        self.writeColored(config.get("gui", "color_nick_buddy"), name, message, self.buddy.dpstatus)
    ...
    def onSend(self, evt):
        ...
        if self.buddy.status not in  [tc_client.STATUS_OFFLINE, tc_client.STATUS_HANDSHAKE]:
            ...
            self.writeColored(config.get("gui", "color_nick_myself"),
                              ...
                              text, self.buddy.dpstatus)
        else:
            self.buddy.storeOfflineChatMessage(text)
            self.writeColored(config.get("gui", "color_nick_myself"),
                              config.getProfileName(),
                              "[%s] %s" % (lang.NOTICE_DELAYED, text), self.buddy.dpstatus)
    ...
    def onBuddyAvatarChanged(self):
        #JM FIXME - Insert a line w/note
        pass

    def onBuddyProfileChanged(self):
        #JM FIXME - Insert a line w/note and details
        #
        #JM ask if local usere wants to replace buddy name with new profile name
    ...
    def onActivateLog(self, evt):
        ...
        self.writeColored(config.get("gui", "color_nick_myself"), "***", "[%s]" % lang.LOG_STARTED, self.buddy.dpstatus)
    ...
    def onStopLog(self, evt):
        ...
        cur = os.path.join(config.getDataDir(), "%s.log" % self.buddy.address)
        if os.path.exists(cur):
            self.writeColored(config.get("gui", "color_nick_myself"), "***", "[%s]" % lang.LOG_STOPPED, self.buddy.dpstatus)
            shutil.move(cur, old)
...
class MainWindow(wx.Frame):
    def __init__(self, socket=None):
        ...
        self.SetTitle("tor.chat: %s" % config.getProfileLongName())
