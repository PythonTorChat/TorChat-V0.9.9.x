...
STATUS_CHECK = "check_log_status"
STATUS_AVAIL = "available"
STATUS_AWAY  = "away"
STATUS_XAWAY = "xa"
STATUS_GOOD  = "2"

EXT_STATUS_OK    = 1	# versions match
EXT_STATUS_OLD   = 2	# buddy using a newer version, you outta look into that
EXT_STATUS_OK_1  = 3
EXT_STATUS_OLD_1 = 4
...
class Buddy(object):
    """Represets a buddy. Every buddy on the buddy list will have such 
    ...
    def __init__(self, address, buddy_list, name=u"", temporary=False):
        ...
        self.profile_decor = config.DEFAULT_DECOR	   # default to V0.9... appearance
        self.profile_address = self.decorateAddress()	
        self.random1 = str(random.getrandbits(256))
        self.random2 = str(random.getrandbits(256))
        self.conn_out = None
        self.conn_in = None
        self.status = STATUS_OFFLINE
        self.ext_status = EXT_STATUS_OK
        self.client = ""
        self.version = ""
    ...
    def onProfileName(self, name):
        print "(2) %s.onProfileName" % self.address
        self.profile_name = name
        if self.name == "" and name != "":  #MW - This is a one-shot fill in a blank name.
            self.name = name
            self.bl.save()
        self.bl.gui(CB_TYPE_PROFILE, self)

    def onProfileDecor(self, decor):
        print "(2) %s.onProfiledecor" % self.address
        if decor != "" :
            self.profile_decor = decor
            self.profile_address = self.decorateAddress()
            self.bl.save()
        self.bl.gui(CB_TYPE_PROFILE, self)
    ...
    def onVersion(self, version):
        ...
        versions = sversion.split(".")	#MW -- note, it "assumes" x.y.z.b
        self.version_major = int(versions[0])*1000 + int(versions[1])
        self.version_minor = int(versions[2])*1000 + int(versions[3])
        self.sendProfile()

    def onProfileText(self, text):
        print "(2) %s.onProfileText" % self.address
        # is a departure from Ver 0.9.9.554
        if (self.version_minor > config.MINORVERSION_009554) and (self.version_minor != 0):
            self.dpstatus = text[0:1]
            self.profile_text = text[1:]
            self.bl.save()
        else:
    ...
    def getAddressAndDisplayName(self):
        if self.name == u"":
            ...
            return self.name + u" (" + self.address + u")"
    ...
    def decorateAddress(self):	
        # Address decoration is a mix of upper/lower case only. Coloration done in GUI
				# Old fashioned index through 2 strings side-by-side
        if len(self.address) == len(self.profile_decor) :
            ndx=0
            decname = u""
            while ndx < len(self.address):
                d = self.profile_decor[ndx]
                c = self.address[ndx]
                if d not in "ABCGIKLMRSUWY":  # Upper / Lower case. Default lower. (FIXME config.DECOR_SET)
                    decname += c.lower()
                else:
                    decname += c.upper()
                ndx +=1
    ...
    def sendProfile(self):
        if self.isAlreadyPonged():
            ...
            #MW - Send the blanks. User should have ability to delete this from remote end
            if name != "":
                msg = ProtocolMsg_profile_name(self, name.encode("UTF-8"))
            else:
                msg = ProtocolMsg_profile_name(self, " ") # send a blank, *then* a null to wipe the old profile name
                msg = ProtocolMsg_profile_name(self, u"")  
            msg.send()

            # this message is optional
            if self.version_major >= config.MAJORVERSION_000010:
                decor = config.get("profile", "decor")
                self.profile_decor = decor
                #MW - Fix a missing ini file decor
                if decor != "":
                    msg = ProtocolMsg_profile_decor(self, decor)
                else:
                    msg = ProtocolMsg_profile_decor(self, config.DEFAULT_DECOR)
                msg.send()

            # this message is optional
            if self.version_major >= config.MAJORVERSION_000010:
                text = self.dpstatus + config.get("profile", "text")
            else:
            ...
            if text != "":
                ...
                msg = ProtocolMsg_profile_text(self, u" ")  # send a blank, *then* a null to wipe the old profile text
                msg = ProtocolMsg_profile_text(self, u"")  
            msg.send()
    ...
    def getDisplayName(self):
        #MW - DO NOT TOUCH!! A fair bit of hard-coded logic depends on this one.
        if self.name == u"":
            line = self.profile_address
        else:
            line = self.profile_address + u" (" + self.name + u")"
...
class BuddyList(object):
    ...
    def save(self):
        #
        # MW - Completely revamped for buddylist.ini
        #
        #
        #f = open(os.path.join(config.getDataDir(), "buddy-list.txt"), "w")
        #MW Save ourself first in list always.
        #            
        #line = ("%s%s %s\r\n" % (self.own_buddy.address, self.own_buddy.dpstatus, self.own_buddy.name)).encode("UTF-8")           
        #f.write(line)
        #
        # Save buddy list w/ dp status here
        #for buddy in self.list:
        #    if buddy.address != self.own_buddy.address:
        #        if buddy.name == "":
        #            line = ("%s%s \r\n" % (buddy.address, buddy.dpstatus)).encode("UTF-8")
        #        else:
        #            line = ("%s%s %s\r\n" % (buddy.address, buddy.dpstatus, buddy.name)).encode("UTF-8") 
        #        f.write(line)
        #            
        #f.close()
        #
...
#
    ...
    def removeBuddy(self, buddy_to_remove, disconnect=True):
        ...
        file_name = os.path.exists(os.path.join(config.getDataDir(), '%s.log' % buddy_to_remove))
        try:
            wipeFile(file_name)
        except:
            pass

        file_name = os.path.exists(os.path.join(config.getDataDir(), 'disabled_%s.log' % buddy_to_remove))
        try:
            wipeFile(file_name)
        except:
            pass
    ...
    def getBuddyFromAddress(self, address):
        # MW: OK mixed case decoration fouled this up....
        test = str(address)
        for buddy in self.list:
            btest = str(buddy.address)
            if btest.lower() == test.lower():
                return buddy
        return None
...
class ProtocolMsg_ping(ProtocolMsg):
    ...
    def execute(self):
        ...
        if self.buddy.dpstatus != STATUS_GOOD:
            stat = int(self.buddy.dpstatus) -1
            self.buddy.dpstatus = str(stat)
...
class ProtocolMsg_version(ProtocolMsg):
    ...
    def execute(self):
        if self.buddy:
            ...
            # split major and minor versions here.
            self.buddy.onVersion(self.version)
...
class ProtocolMsg_status(ProtocolMsg):
    ...
    def execute(self):
        ...
        if self.buddy:
            ...
            isMe = (self.buddy.address == self.bl.own_buddy.address)
            isOK = (self.bl.own_buddy.dpstatus == STATUS_GOOD)
            ...
            #set buddy status
            if self.status == STATUS_AVAIL:
                if self.buddy.dpstatus == STATUS_GOOD or isMe or not isOK:
                    self.buddy.onStatus(STATUS_ONLINE)
                else:
                    self.buddy.onStatus(STATUS_ONLINE_LOG)
            if self.status == STATUS_AWAY:
                if self.buddy.dpstatus == STATUS_GOOD or isMe or not isOK:
                    self.buddy.onStatus(STATUS_AWAY)
                else:
                    self.buddy.onStatus(STATUS_AWAY_LOG)
            if self.status == STATUS_XAWAY:
                if self.buddy.dpstatus == STATUS_GOOD or isMe or not isOK:
                    self.buddy.onStatus(STATUS_XA)
                else:
                    self.buddy.onStatus(STATUS_XA_LOG)

            #avoid timeout of in-connection
            self.connection.last_active = time.time()


class ProtocolMsg_extended_status(ProtocolMsg):
    """transmit the extended status block, this will be be sent 
    after 120 seconds and then once per hour
    When receiving this message the client will compare the client
    version with buddy and if running a lower version number will
    pop-up a notice that there is a newer TorChat version available"""
    def parse(self):
        self.ext_status = self.blob
        
    def execute(self):
        #set the ext_status field of the corresponding buddy
        if self.buddy:
            print "(3) received extended status %s from %s" % (self.ext_status, self.buddy.address)

            #set buddy status
            if self.ext_status == STATUS_AVAIL:
                if self.buddy.dpstatus == STATUS_GOOD:
                    self.buddy.onStatus(STATUS_ONLINE)
                else:
                    self.buddy.onStatus(STATUS_ONLINE_LOG)

            #throttle-down messages whilst in the connection
            self.connection.last_ext_status = time.time()
...
class ProtocolMsg_profile_decor(ProtocolMsg):
    """transmit the text that is set in the pofile address decoration (this message is optional)"""
    def parse(self):
        self.decor = self.blob.decode("UTF-8")
        
    def execute(self):
        if self.buddy:
            print "(2) received profile decor from %s" % self.buddy.address
            self.buddy.onProfileDecor(self.decor)
...
class InConnection(object):
    def __init__(self, socket, buddy_list):
        ...
        self.last_ext_status = 0	# flag to send after 1st 2 min cycle, once per hour thereafter
        self.started = True
        ...
        if tor_pid:
            ...
            while cnt <= 20:
                try:
                    ...
                    config.cset("client", "own_hostname", hostname)
                    found = True
                    f.close()
                    break
                except:
