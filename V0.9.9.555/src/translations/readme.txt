...
lang_en.py is the template language file that will have all
text phrases for translation. If a language file misses a phrase, 
tor.chat will just use the english version in lang_en.py 

If you have a new translation file you wish to test, then add it to this
directory and add the filename to the imports statements in __init__.py. 
...
- add the filename to the imports statements in __init__.py and
  and place the new copy of __init__.py *outside* this folder, 
  next to torchat.exe or torchat.py

- change the language code in torchat.ini to the new one

- start translating the strings in your new lang_xx.py

- start tor.chat with log_level 2 and look into the log file
...
- run tor.chat with loglevel 2 to get a list of the missing strings.
...
- Add your name to the header of the file, to receive much fame and immortality, 
  email it to haxxie@safe-mail.net and I will include it in the next release.
