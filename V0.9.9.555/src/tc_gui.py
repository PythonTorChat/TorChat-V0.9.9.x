...
ICON_NAMES = {tc_client.STATUS_OFFLINE : "offline.png",
              tc_client.STATUS_OFFLINE_LOG : "offline-l.png",
              tc_client.STATUS_ONLINE : "online.png",
              tc_client.STATUS_ONLINE_LOG : "online-l.png",
              tc_client.STATUS_HANDSHAKE : "connecting.png",
              tc_client.STATUS_HANDSHAKE_LOG : "connecting-l.png",
              tc_client.STATUS_AWAY : "away.png",
              tc_client.STATUS_AWAY_LOG : "away-l.png",
              tc_client.STATUS_XA : "xa.png",
              tc_client.STATUS_XA_LOG : "xa-l.png"}
...
class TaskbarIcon(wx.TaskBarIcon):
    ...
    def getToolTipText(self):
        text = "%s - TorChat" % config.getProfileLongName()
...
#        self.AppendSeparator()
    ...
    def onDelete(self, evt):
        ...
        if answer == wx.YES and self.buddy.address != config.get("client","own_hostname"):
...
#
#MW - Just open the chat window. If there were a bunch of messages the messagebox gets ugly
...
#        wx.MessageBox(om, lang.MSG_OFFLINE_TITLE, wx.ICON_INFORMATION)
    ...
    def onAbout(self, evt):
        wx.MessageBox(lang.ABOUT_TEXT % {"version":version.VERSION,
                                         "svn":version.VERSION_BUILD,
                                         "copyright":config.COPYRIGHT,
                                         "python":".".join(str(x) for x in sys.version_info),
                                         "wx":wx.version(),
                                         "tipjar":config.TIP_JAR},
                      lang.ABOUT_TITLE)

    def onTipJar(self, evt):
        copyTipJar = wx.MessageBox(lang.TIPJAR_TEXT % {"tipjar":config.TIP_JAR},
                                   lang.TIPJAR_TITLE,
                                   wx.YES_NO|wx.YES_DEFAULT|wx.CANCEL)
        if copyTipJar == wx.YES:
            if not wx.TheClipboard.IsOpened():
                cliptext = str(config.TIP_JAR)
                clipdata = wx.TextDataObject()
                clipdata.SetText(cliptext)
                wx.TheClipboard.Open()
    ...
    def onAskAuthor(self, evt):
        if self.mw.buddy_list.getBuddyFromAddress(config.get("branding", "support_id2")):
            wx.MessageBox(lang.DEC_MSG_ALREADY_ON_LIST % config.get("branding", "support_name2"))
        else:
...
class DlgEditContact(wx.Dialog):
    def __init__(self, parent, main_window, buddy=None, add_author=False): #no buddy -> Add new
        ...
        if not self.buddy:
            ...
            self.txt_intro = wx.TextCtrl(self.panel, -1, "greetings...")
            self.txt_intro.SetMinSize((250, -1))
            sizer.Add(self.txt_intro, (row, 1), (1, 2))

        if add_author:
            self.txt_address.SetValue(config.get("branding", "support_id2"))
            self.txt_name.SetValue(config.get("branding", "support_name2"))
...
#                wx.MessageBox(lang.DEC_MSG_ALREADY_ON_LIST % address)
                buddy.name = self.txt_name.GetValue()
                self.bl.save()
            else:
...
class DlgEditProfile(wx.Dialog):
    ...
    def onOk(self, evt):
        config.cset("profile", "name", self.txt_name.GetValue())
        config.cset("profile", "text", self.txt_text.GetValue())
        ...
        for buddy in self.mw.buddy_list.list:
            #MW FIXME - also stuff profile name into buddy list for myself or revert to "myself" if null
            buddy.sendProfile()
            buddy.sendStatus()
...
class BuddyList(wx.ListCtrl):
    def __init__(self, parent, main_window):
        ...
        for status in [tc_client.STATUS_OFFLINE,
                       tc_client.STATUS_OFFLINE_LOG,
                       tc_client.STATUS_HANDSHAKE,
                       tc_client.STATUS_HANDSHAKE_LOG,
                       tc_client.STATUS_ONLINE,
                       tc_client.STATUS_ONLINE_LOG,
                       tc_client.STATUS_AWAY,
                       tc_client.STATUS_AWAY_LOG,
                       tc_client.STATUS_XA,
                       tc_client.STATUS_XA_LOG]:
    ...
    def onTimer(self, evt):
        self.blink_phase = not self.blink_phase
        self.blink_offCnt += 1
        if self.blink_offCnt > 9:
            self.blink_offCnt = 0     # slower blink for off-line messages
        # blink all buddies with hidden chat windows
        for buddy in self.bl.list:
            if buddy.getOfflineMessages():
                self.blinkBuddy(buddy, True)
#            else:
#                self.blinkBuddy(buddy, False)    # ensure "blink" was cleared
            for window in self.mw.chat_windows:
                if window.buddy == buddy:
                    if not window.IsShown():
                        self.blinkBuddy(window.buddy, True)
                    else:
                        self.blinkBuddy(window.buddy, False)    # ensure "blink" was cleared
    ...
    def onBuddyStatusChanged(self, buddy):
        ...
        #MW Update own status as well
        line = self.bl.own_buddy.getDisplayName()
        index = self.FindItem(0, line)
        if index != -1:
            bstatus = self.bl.own_buddy.status
            self.SetItemImage(index, self.il_idx[bstatus])
    ...
    def onBuddyProfileChanged(self, buddy):
        ...
        #MW - Refresh the buddy list for name decor
        self.onListChanged()
    ...
    def onListChanged(self):
        ...
        #MW FIXME - Put own address in top line always, then a seperator, then buddies
        #MW - Clear the list and re-populate it from the buddylist file
    ...
    def getBuddyFromIndex(self, index):
        ...
        for buddy in self.bl.list:
            if buddy.getDisplayName() == name:
                return buddy
            elif buddy.profile_address == name:
                return buddy
...
#MW Fixme - This is where the mods for position-by-buddy code will go in future revision.
...
#        return t[:-19]     # no longer works as hard-coded trim.
    ...
    def writeColored(self, color, name, text, textColor, stat):
        ...
        if stat == tc_client.STATUS_GOOD:
            self.txt_in.write("%s " % time.strftime(config.get("gui", "time_stamp_format")))
        else:
            self.txt_in.write("%s " % time.strftime(config.get("gui", "time_stamp_formatx")))
...
#            self.txt_in.SetDefaultStyle(wx.TextAttr(wx.SystemSettings.GetColour(wx.SYS_COLOUR_WINDOWTEXT)))
        self.txt_in.SetDefaultStyle(wx.TextAttr(textColor))
        ...
        if os.path.exists(os.path.join(config.getDataDir(), "%s.log" % self.buddy.address)):
            logtext = "%s %s: %s" % (time.strftime("(%Y-%m-%d %H:%M:%S)"), name, text)
            self.log(logtext)
    ...
    def notifyOfflineSent(self):
        ...
        nickColor = config.get("gui", "color_nick_myself")
        textColor = config.get("gui", "color_text_myself")
        self.writeColored(nickColor, config.getProfileName(), message, textColor, self.buddy.dpstatus)
    ...
    def process(self, message):
        ...
        if self.buddy.name != "":
            ...
            name = self.buddy.profile_address
        nickColor = config.get("gui", "color_nick_buddy")
        textColor = config.get("gui", "color_text_buddy")
        self.writeColored(nickColor, name, message, textColor, self.buddy.dpstatus)
    ...
    def onClose(self, evt):
        w,h = self.GetSize()
#        x,y = self.GetPosition() #MW - FIXME - Future for buddylist.ini remember chat window size & pos per buddy
        config.cset("gui", "chat_window_width", w)
        config.cset("gui", "chat_window_height", h)
        config.cset("gui", "chat_window_height_lower", h - self.splitter.GetSashPosition())
    ...
    def onSend(self, evt):
        ...
        if self.buddy.status not in  [tc_client.STATUS_OFFLINE, tc_client.STATUS_HANDSHAKE]:
            ...
            self.writeColored(nickColor, config.getProfileName(),
                              text, textColor, self.buddy.dpstatus)
        else:
            self.buddy.storeOfflineChatMessage(text)
            self.writeColored(nickColor, config.getProfileName(),
                              "[%s] %s" % (lang.NOTICE_DELAYED, text), 
                              textColor, self.buddy.dpstatus)
    ...
    def onURL(self, evt):
        ...
        elif evt.GetMouseEvent().GetEventType() == wx.wxEVT_RIGHT_DOWN:

            self.UrlFrom = evt.GetURLStart()
            self.UrlTo = evt.GetURLEnd()
            empty = (self.UrlFrom == self.UrlTo)
            if not empty:
                menu = wx.Menu()
                id = wx.NewId()
    ...
    def onCopyUrl(self, evt):
        ...
        if empty:
            return
        #MW - FIXME Threshold test here
        text = self.txt_in.GetRange(self.UrlFrom, self.UrlTo)
        clipdata = wx.TextDataObject()
        clipdata.SetText(text)
        wx.TheClipboard.Open()
        wx.TheClipboard.SetData(clipdata)
    ...
    def onBuddyAvatarChanged(self):
        #MW FIXME - Insert a line w/note
        pass

    def onBuddyProfileChanged(self):
        #MW FIXME - Insert a line w/note and details
        #MW ini file flag to ask or accept?
        #MW ask if local usere wants to replace buddy name with new profile name
    ...
    def onActivateLog(self, evt):
        ...
        self.writeColored(nickColor, "****", "[%s]" % lang.LOG_STARTED, textColor, self.buddy.dpstatus)

    def onStopLog(self, evt):
        old = os.path.join(config.getDataDir(), "disabled_%s.log" % self.buddy.address)
        #self.buddy.dpstatus="2"
        cur = os.path.join(config.getDataDir(), "%s.log" % self.buddy.address)
        if os.path.exists(cur):
            nickColor = config.get("gui", "color_nick_myself")
            textColor = config.get("gui", "color_text_myself")

            self.writeColored(nickColor, "****", "[%s]" % lang.LOG_STOPPED, textColor, self.buddy.dpstatus)
            shutil.move(cur, old)
...
class MainWindow(wx.Frame):
    def __init__(self, socket=None):
        wx.Frame.__init__(
            ...
            -1,
            "TorChat",
            pos=(config.getint("gui", "main_window_h_pos"),
                 config.getint("gui", "main_window_v_pos")),
            size=(
        ...
        self.SetTitle("%s - TorChat" % config.getProfileLongName())
    ...
    def exitProgram(self):
        ...
        config.cset("gui", "main_window_width", w)
        config.cset("gui", "main_window_height", h)
        config.cset("gui", "main_window_h_pos", x)
        config.cset("gui", "main_window_v_pos", y)
        found_unread = False
        for window in self.chat_windows:
